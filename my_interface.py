...
FSM_STATES = ["SafeMode", "PanicMode", "ManualMode", "CalibrationMode",
              "YawControlMode", "FullControlMode", "RawMode"]
            ...
            if not MOCK_MODE:
                ...
                if time.time() - mock_p_timer > 2:
                    for axis in ["yaw", "pitch", "roll"]:
                        p_values[axis] = round(p_values[axis] + random.uniform(-0.05, 0.05), 3)
                    mock_p_timer = time.time()
...
with dpg.window(label="Drone Ground Station", tag="main_window", width=900, height=800):
    ...
    with dpg.group(horizontal=True):
        ...
        with dpg.child_window(width=580, height=940, border=False):
            ...
            with dpg.table(tag="msg_table", header_row=True,
                           borders_innerH=True, borders_outerH=True,
                           borders_innerV=True, borders_outerV=True,
                           scrollY=True, freeze_rows=1,
                           height=280):
                dpg.add_table_column(label="Time",      width_fixed=True, init_width_or_weight=70)
                dpg.add_table_column(label="Direction", width_fixed=True, init_width_or_weight=90)
