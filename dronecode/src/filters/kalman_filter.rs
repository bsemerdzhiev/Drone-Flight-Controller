...
use crate::util::constants_file::{ChosenFixedPointType, DEGREE_TO_RAD, RAD_TO_DEGREE};
...
const C1: ChosenFixedPointType = ChosenFixedPointType::lit("1.4");
const C2: ChosenFixedPointType = ChosenFixedPointType::lit("100000");

pub struct KalmanFilter {
    bias_p: ChosenFixedPointType,
    bias_q: ChosenFixedPointType,
    reading: (Axis<I32F0>, Axis<I32F0>),
    last_read_time: Instant,
    pub roll: ChosenFixedPointType,
    pub pitch: ChosenFixedPointType,
    yaw: ChosenFixedPointType,
...
impl KalmanFilter {
    pub fn new(offset: (Axis<I32F0>, Axis<I32F0>)) -> Self {
        KalmanFilter {
            bias_p: ChosenFixedPointType::from_num(0),
            bias_q: ChosenFixedPointType::from_num(0),
            reading: (
                Axis::<I32F0> {
                    x: I32F0::from_num(0),
                    y: I32F0::from_num(0),
    ...
    fn update_roll(&mut self, dt: ChosenFixedPointType) {
        let p_clean =
            (ChosenFixedPointType::from_num(self.reading.1.x) * DEGREE_TO_RAD) - self.bias_p;
        // let raw_roll = atan2f(self.reading.0.y as f32, self.reading.0.z as f32);
        let raw_roll =
            ChosenFixedPointType::from_num(approx_atan2(self.reading.0.y, self.reading.0.z));
    ...
    fn update_pitch(&mut self, dt: ChosenFixedPointType) {
        let ay = ChosenFixedPointType::from_num(self.reading.0.y);
        let az = ChosenFixedPointType::from_num(self.reading.0.z);

        let q_clean =
            (ChosenFixedPointType::from_num(self.reading.1.y) * DEGREE_TO_RAD) - self.bias_q;
        let raw_pitch = approx_atan2(
            ChosenFixedPointType::from_num(-self.reading.0.x),
    ...
    fn update_yaw(&mut self, dt: ChosenFixedPointType) {
        ...
        self.yaw = ChosenFixedPointType::from_num(self.reading.1.z) * dt;
    }
}
...
impl ImuHandler for KalmanFilter {
    fn append_new_reading(&mut self) {
        ...
        let dt = ChosenFixedPointType::from_num(
            cur_time
                .duration_since(self.last_read_time)
                .as_secs_f32()
                .clamp(0.001, 0.03),
