 ...
 */
typedef enum {
  BOOTLOADER_UPDATING, /**< Bootloader status for indicating that an update is
                          in progress. */
  BOOTLOADER_SETTINGS_SAVING, /**< Bootloader status for indicating that saving
                                 of bootloader settings is in progress. */
  BOOTLOADER_COMPLETE, /**< Bootloader status for indicating that all operations
                          for the update procedure has completed and it is safe
                          to reset the system. */
  BOOTLOADER_TIMEOUT, /**< Bootloader status field for indicating that a timeout
                         has occured and current update process should be
                         aborted. */
  BOOTLOADER_RESET, /**< Bootloader status field for indicating that a reset has
                       been requested and current update process should be
                       aborted. */
} bootloader_status_t;

static pstorage_handle_t
    m_bootsettings_handle; /**< Pstorage handle to use for registration and
                              identifying the bootloader module on subsequent
                              calls to the pstorage module for load and store of
                              bootloader setting in flash. */
static bootloader_status_t
    m_update_status; /**< Current update status for the bootloader module to
                        ensure correct behaviour when updating settings and when
                        update completes. */
 ...
 * detailed description of the parameters provided with the callback, please
 * refer to \ref pstorage_ntf_cb_t.
 */
static void pstorage_callback_handler(pstorage_handle_t *p_handle,
  ...
  // If we are in BOOTLOADER_SETTINGS_SAVING state and we receive an
  // PSTORAGE_STORE_OP_CODE response then settings has been saved and update has
  // completed.
 ...
 *
 * @details This function will place the chip in low power mode while waiting
 * for events from the SoftDevice or other peripherals. When interrupted by an
 * event, it will call the
 *          @ref app_sched_execute function to process the received event. This
 * function will return when the final state of the firmware update is reached
 * OR when a tear down is in progress.
 */
  ...
  if (p_bootloader_settings->bank_0 == BANK_VALID_APP) {
    ...
    if (p_bootloader_settings->bank_0_crc != 0) {
      image_crc = crc16_compute((uint8_t *)DFU_BANK_0_REGION_START,
                                p_bootloader_settings->bank_0_size, NULL);
    }
...
static void bootloader_settings_save(bootloader_settings_t *p_settings) {
  ...
  err_code = pstorage_store(&m_bootsettings_handle, (uint8_t *)p_settings,
                            sizeof(bootloader_settings_t), 0);
  APP_ERROR_CHECK(err_code);
}
...
void bootloader_dfu_update_process(dfu_update_status_t update_status) {
  ...
  if (update_status.status_code == DFU_UPDATE_APP_COMPLETE) {
    ...
    settings.bank_0_crc = update_status.app_crc;
    settings.bank_0_size =
        update_status.sd_size + update_status.bl_size + update_status.app_size;
    ...
    // This handles cases where SoftDevice was not updated, hence bank0 keeps
    // its settings.
    else {
      settings.bank_0 = p_bootloader_settings->bank_0;
      settings.bank_0_crc = p_bootloader_settings->bank_0_crc;
...
/**@brief Function for disabling all interrupts before jumping from bootloader
 * to application.
 */
...
void bootloader_app_start(uint32_t app_addr) {
  // If the applications CRC has been checked and passed, the magic number will
  // be written and we can start the application safely.
...
uint32_t bootloader_dfu_sd_update_continue(void) {
  ...
  // Ensure that flash operations are not executed within the first 100 ms
  // seconds to allow a debugger to be attached.
  nrf_delay_ms(100);
