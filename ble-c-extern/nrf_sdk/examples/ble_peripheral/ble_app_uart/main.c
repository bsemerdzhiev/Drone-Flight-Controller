...
#define APP_ADV_TIMEOUT_IN_SECONDS                                             \
  180 /**< The advertising timeout (in units of seconds). */
...
#define FIRST_CONN_PARAMS_UPDATE_DELAY                                         \
  APP_TIMER_TICKS(                                                             \
      5000, APP_TIMER_PRESCALER) /**< Time from initiating event (connect or   \
...
#define START_STRING                                                           \
  "Start...\n" /**< The string that will be sent over the UART when the        \
                  application starts. */
...
/**@brief Function for dispatching a S110 SoftDevice event to all modules with a
 * S110 SoftDevice event handler.
 *
 * @details This function is called from the S110 SoftDevice event interrupt
 * handler after a S110 SoftDevice event has been received.
 *
 * @param[in] p_ble_evt  S110 SoftDevice event.
 */
 ...
 * the last character received was a 'new line' i.e '\n' (hex 0x0D) or if the
 * string has reached a length of
...
static void buttons_leds_init(bool *p_erase_bonds) {
  ...
  uint32_t err_code =
      bsp_init(BSP_INIT_LED | BSP_INIT_BUTTONS,
...
/**@brief Function for placing the application in low power state while waiting
 * for events.
 */
